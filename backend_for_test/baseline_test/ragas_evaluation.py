...
Medical RAG - Answer Evaluation Module (DeepSeek + ragas)

Overall Flow:
1. First, run `rag_generate_and_ragas_test` to get `generated_answers.json`.
2. This script will:
- Read `generated_answers.json`
- Add a standard answer reference for each question
- Generate `ragas_eval_input.json` (containing `user_input`, `response`, `retrieved_contexts`, and `reference`)
- Call ragas + DeepSeek for evaluation and output various metrics.
...
Evaluation Metrics:
- LLMContextRecall: Contextual recall
- Faithfulness: Faithfulness (whether the generated answer is faithful to the retrieved context)
- Factual Correctness: The degree of factual accuracy (consistency with the standard answer)
"""
...
# ====== rag_generate_and_ragas_test. Configuration area: Can be modified as needed. ===============================

# Files generated by answer_generation.py
...
# DeepSeek API Key (can also be provided via the environment variable DEEPSEEK_API_KEY)
DEEPSEEK_API_KEY = os.getenv("DEEPSEEK_API_KEY") or "sk-3a59029618234b1496de29504891bf78"

# ===============================================================

# ragas + langchain related dependencies
try:
...
# ---------------------------------------------------------------------
# 2. The standard answer to the sample question (reference)
# ---------------------------------------------------------------------
# Note:
#   - These 18 questions correspond one-to-one with your current rewritten_query.json
#   - build_ragas_input will sequentially align the i-th item in generated_answers.json
#   with REFERENCE_LIST[i] to form an evaluation sample with references.
# ---------------------------------------------------------------------
...
def load_generated_answers(path: str) -> List[Dict[str, Any]]:
    ...
    if not isinstance(data, list):
        raise ValueError(f"The {path} top-level JSON must be a list.")
    ...
    for idx, item in enumerate(data):
        ...
        for key in ("user_input", "response", "retrieved_contexts"):
            if key not in item:
                raise ValueError(f"The record {idx} in {path} is missing the field: {key}")
...
def build_ragas_input(
    ...
    """
    Align `generated_answers.json` with `REFERENCE_LIST` to generate the required structure for `ragas_eval_input.json`.

    Alignment strategy:
    - By default, they correspond one-to-one in "order": the i-th generated answer corresponds to the i-th reference.
    - If the lengths of the two are different, the smaller value is used and a prompt is given.
    """
    n = min(len(generated), len(references))
    if len(generated) != len(references):
        print(
            f"[RAGAS] Warning: generated_answers.json contains {len(generated)} entries."
            f"The REFERENCE_LIST has {len(references)} entries and will be aligned to the first {n} entries."
        )
...
# ---------------------------------------------------------------------
# 4. Evaluation using ragas + DeepSeek
# ---------------------------------------------------------------------
...
def run_ragas(
    ...
    if evaluate is None or LangchainLLMWrapper is None or Dataset is None:
        raise ImportError(
            "ragas or datasets are not installed correctly. Please run:\n"
            "  pip install 'ragas>=0.3.0' 'datasets>=2.0.0'"
        )
    if LLMContextRecall is None or Faithfulness is None or FactualCorrectness is None:
        raise ImportError(
            "The ragas.metrics file was not imported correctly. Please check your ragas version."
        )
    if init_chat_model is None:
        raise ImportError(
            "Langchain is not installed. Please run the following command first:\n"
    ...
    if not deepseek_api_key or "在这里填你的DeepSeek_APIKey" in deepseek_api_key:
        raise ValueError(
            "No DeepSeek API Key provided. Please enter your key in the DEEPSEEK_API_KEY field at the top of the file."
            "Alternatively, provide it via the environment variable DEEPSEEK_API_KEY."
        )

    if not ragas_samples:
        raise ValueError("ragas_samples is empty, so it cannot be evaluated.")

    # rag_generate_and_ragas_test. construct HuggingFace Dataset
    dataset = Dataset.from_list(ragas_samples)  # type: ignore[arg-type]

    # 2. Initialize DeepSeek as an evaluation LLM
    llm = init_chat_model(
        model="deepseek-chat",
    ...
    # 4. Convert the results into a simple {metric: score} dictionary.
...
def main() -> None:
    ...
    # rag_generate_and_ragas_test. read generated_answers.json
    try:
        generated = load_generated_answers(GENERATED_ANSWERS_PATH)
    except Exception as e:
        print(f"[RAGAS] read {GENERATED_ANSWERS_PATH} fail：{e}")
        return

    print(f"[RAGAS] Successfully read generated_answers.json, a total of {len(generated)} samples.")

    # 2. construct ragas_eval_input.json
    ragas_samples = build_ragas_input(generated, REFERENCE_LIST)
    save_json(ragas_samples, RAGAS_INPUT_PATH)
    print(f"[RAGAS] Evaluation data has been written to: {RAGAS_INPUT_PATH}")

    # 3. call ragas + DeepSeek for evaluation
    try:
        metrics = run_ragas(ragas_samples, deepseek_api_key=DEEPSEEK_API_KEY)
    except ImportError as e:
        print("The [RAGAS] dependency is not installed, and the evaluation cannot be performed.")
        print("error message：", e)
        return
    except Exception as e:
        print("[RAGAS] Error occurred while calling ragas for evaluation:", e)
        print("If you only need to generate ragas_eval_input.json for the time being, you can ignore this error.")
        return

    # 4. Print and save the evaluation results
    if metrics:
        print("\n[RAGAS] Evaluation results (average score for each indicator):")
        ...
        print(f"\n[RAGAS] evaluation results have been written: {RAGAS_REPORT_PATH}")
    else:
        print("The [RAGAS] evaluation returned an empty result. Please check your ragas version or input data.")
